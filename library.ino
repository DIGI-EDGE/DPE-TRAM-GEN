...
void setup() {
  ...
  Header_init(&header, dev_add, dev_eui, app_eui);
   ...
   byte rssi_value[] = {0x63, 0x66, 0x75, '\0'}; //here we just have one byte
   // 2. then a varuable contain the type
   byte rssi_type = RSSI_TYPE;
   AppendInHeader(&header,rssi_type, rssi_value);
  
   // the type can be costumized
   // 1. creat an array holds the data
   byte rand_value[] = {0x63, 0x75, '\0'}; //here we just have one byte
   // 2. then a varuable contain the type
   AppendInHeader(&header,0x09, rand_value);

   //we will append for exemple Battery data to header
   // 1. creat an array holds the data
   byte battery_value[] = {0x60, 0x62, '\0'}; //here we just have two bytes
   // 2. then a varuable contain the type
